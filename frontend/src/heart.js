...
export class HeartView {
  ...
  resize() {
    ...
    // Frame from the geometry's own extent rather than a fixed field of view.
    // A magic number has to be re-tuned by hand every time the anatomy
    // changes, and when it is not the aortic arch quietly runs off the top of
    // the panel -- which is exactly what happened when the silhouette was
    // reshaped. _frameToBounds owns both aspect and fov.
    this._frameToBounds(w, h);
  }

  /**
   * Choose a field of view that fits the whole organ with a margin.
   *
   * Vertical extent is the binding constraint in a wide panel and horizontal
   * extent in a narrow one, so both are solved and the wider view wins. The
   * margin leaves room for the contraction, which briefly pushes the surface
   * out past its rest bounds -- framing to the rest pose alone makes the
   * silhouette clip on every beat.
   */
  _frameToBounds(w, h) {
    const aspect = w / h;
    this.camera.aspect = aspect;

    const r = this.stats?.bounds?.enclosing;
    if (!r) {
      this.camera.fov = aspect < 0.9 ? 34 : 28;
      this.camera.updateProjectionMatrix();
      return;
    }

    // Fit a sphere of radius r centred on the origin. Because that sphere is
    // rotation-invariant the fit holds however the user has spun the organ,
    // and the margin can therefore be tight rather than defensive.
    //
    // 1.06 leaves room for systole pushing the surface out past its rest
    // extent, plus a little breathing space at the panel edge.
    const dist = this.camera.position.z;
    const need = r * 1.06;

    const fovForHeight = 2 * Math.atan(need / dist);
    // Horizontal is the binding constraint on a narrow panel; convert to the
    // vertical fov three.js actually wants.
    const fovForWidth = 2 * Math.atan(need / (dist * aspect));
    const fov = Math.max(fovForHeight, fovForWidth) * (180 / Math.PI);

    // Clamped so an extreme panel shape cannot produce a fisheye at one end or
    // a telephoto that flattens the organ into a cut-out at the other.
    this.camera.fov = Math.min(Math.max(fov, 16), 48);
    this.camera.updateProjectionMatrix();
  }
